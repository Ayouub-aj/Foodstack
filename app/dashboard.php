...
try {
    // Note: 'role' column is not in the schema, setting isAdmin to false for now
    $isAdmin = false; 
...
try {
    ...
    if ($isAdmin) {
        ...
        $topStmt = $pdo->query("
            SELECT u.username, u.email, COUNT(p.id_recipe) as recipe_count 
            FROM users u 
            LEFT JOIN recipes p ON u.id_user = p.user_id 
            GROUP BY u.id_user 
...
<div class="hero-section text-center mb-5">
    <div class="container">
        ...
        <p class="lead fw-500 opacity-75"><?php echo $recipeCount; ?> recipes published in your library.</p>
        <div class="mt-4 d-flex justify-content-center gap-3">
            <a href="recipe/create.php" class="btn btn-dark btn-lg px-5 py-3 rounded-pill fw-bold shadow-sm transition-hover">
                + New Recipe
            </a>
            <?php if ($isAdmin): ?>
        ...
        <div class="col-lg-<?php echo $isAdmin ? '8' : '12'; ?>">
            <div class="card shadow rounded-4 border-0 p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h3 class="fw-bold m-0"><?php echo $isAdmin ? 'Global Repository' : 'Your Recipes'; ?></h3>
                    <span class="badge bg-success rounded-pill px-3"><?php echo $isAdmin ? count($allRecipes) : $recipeCount; ?> total</span>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <?php 
                            $displayList = $isAdmin ? $allRecipes : $recipes;
                            foreach ($displayList as $p): 
                            ?>
                                <tr>
                                    <td class="ps-3">
                                        <div class="fw-bold text-dark">
                                            <a href="recipe/view.php?id=<?php echo $p['id_recipe']; ?>" class="text-dark text-decoration-none"><?php echo htmlspecialchars($p['title']); ?></a>
                                        </div>
                                        <?php if ($isAdmin): ?>
                                    ...
                                    <td class="text-end pe-3">
                                        <a href="recipe/edit.php?id=<?php echo $p['id_recipe']; ?>" class="btn btn-sm btn-outline-dark rounded-pill px-3 fw-bold me-1">Edit</a>
                                        <a href="recipe/delete.php?id=<?php echo $p['id_recipe']; ?>" class="btn btn-sm btn-outline-danger rounded-pill px-3 fw-bold">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                ...
                <div class="list-group list-group-flush">
                    <?php foreach ($contributors as $c): ?>
                        <div class="list-group-item bg-transparent d-flex justify-content-between align-items-center py-3">
                            ...
                            <span class="badge bg-dark rounded-pill"><?php echo $c['recipe_count']; ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
